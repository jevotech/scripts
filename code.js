...
function darkside(p1,tr){
	...
	if(p1!="" && p1!=null){
		...
		for(var k=1;k<=ccghm;k++){
			...
			}else var out="Enter the BIN following with the correct format such as : '123456xxxxxxxxxx' ";
		}
		if(jou==2) var out=out+"</xml>";
		else if(jou==3) var out=out+"}";
...
function mkCClist(){
	...
	c[i]="4xxx9x604015xxxx"; cd[i]="Visa-Carta S?Â¬-Unipol Banca (Italy)"; i++;
	c[i]="4xxx9x144046xxxx"; cd[i]="Visa-Carta S?Â¬-Banco di Sardegna (Italy)"; i++;
	c[i]="4xxx9xxx40xxxxxx"; cd[i]="Visa-Carta S?Â¬ (Italy)"; i++;
...
function cleanText(){
	var confirmar=confirm("Do you want to erase the CCs generated ?");
